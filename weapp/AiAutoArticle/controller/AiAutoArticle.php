...
namespace weapp\AiAutoArticle\controller;

use think\Page;
use think\Db;
use app\common\controller\Weapp;
use weapp\AiAutoArticle\model\AiAutoArticleModel;

/**
 * 插件的控制器
 */
class AiAutoArticle extends Weapp
{
    /**
     * 实例化模型
     */
    ...
    public function __construct(){
        parent::__construct();
        $this->model = new AiAutoArticleModel;
        $this->db = Db::name('WeappAiAutoArticle');
    ...
    public function add()
    {
        if (IS_POST) {
            ...
            if (false !== $insertId) {
                ...
                $this->success("操作成功", weapp_url('AiAutoArticle/AiAutoArticle/index'));
            }else{
                $this->error("操作失败");
            }
            exit;
        ...
        if (IS_POST) {
            ...
            if(!empty($post['code'])){
                ...
                if ($r) {
                    ...
                    $this->success("操作成功!", weapp_url('AiAutoArticle/AiAutoArticle/conf'));
                }
            }
            $this->error("操作失败!");
        }

        $row = M('weapp')->where('code','eq','AiAutoArticle')->find();
        $this->assign('row', $row);
