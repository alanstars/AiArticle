...
// 查看大图
function imagesShow(obj, max_width, max_height) {
    var links = $(obj).val() ? $(obj).val() : '';
    links = links ? links : $(obj).attr('src');
    if (!links) {
        showErrorAlert('没有获取到图片地址，请检查图片地址隐藏域');
        return false;
    }
...
// 选择链接
function selectAdLink(obj, funcCallback) {
    var ad_host_id = $(obj).data('ad_host_id') ? $(obj).data('ad_host_id') : 1;
    var ad_link_id = $(obj).data('ad_link_id') ? $(obj).data('ad_link_id') : 0;
    var url = $(obj).data('href');
    if (parseInt(ad_host_id) > 0) url += "&ad_host_id=" + ad_host_id;
    if (parseInt(ad_link_id) > 0) url += "&ad_link_id=" + ad_link_id;
    layer.open({
        type: 2,
        title: '选择链接',
        fixed: true,
        ...
        yes: function(index, layero) {
            ...
            if (link_id > 0) {
                funcCallback({host_id: host_id, link_id: link_id, link_names: link_names});
                // layer.closeAll();
            } else {
                showErrorAlert('请选择链接');
            }
        }
    });
}
